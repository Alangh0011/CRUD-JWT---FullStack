...
const Actualizar = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await axios.put(`http://localhost:8080/producto/update/${id}`, formData, {
                headers: {
    ...
    // Función para obtener los detalles del producto según el id
    const obtenerDetalleProducto = async (id) => {
        try {
            const token = localStorage.getItem('token');
    ...
    // Llamar a la función para obtener los detalles del producto cuando el componente se monte
    useEffect(() => {
        obtenerDetalleProducto(id);
    }, [id]); // Asegurar que se vuelva a llamar cuando el id cambie
    ...
    return (
        ...
        <div className="p-4">
            ...
            <h1 className="text-2xl font-bold mb-4 mt-4">Editar Producto</h1>
            <div className="mt-8">
            ...
            <div className="mt-8">
                ...
                className="active:scale-[.98] active:duration-80 transition-all py-2 rounded-xl bg-green-500 text-white text-lg font-bold hover:scale-[1.1] ease-in-out">Guardar</button>
            </div>
